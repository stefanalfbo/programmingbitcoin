...
func TestBlock(t *testing.T) {
	...
	t.Run("ParseBlock return block with correct values", func(t *testing.T) {
		...
		expectedPreviousBlock := []byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xab, 0x28, 0x36, 0x00, 0x03, 0x58, 0x42, 0x6e, 0xd7, 0x9e, 0x26, 0x94, 0xe6, 0x66, 0x75, 0xa3, 0x05, 0xd2, 0x4f, 0x66, 0x89, 0xcd, 0x1e}
		if !bytes.Equal(block.PreviousBlock[:], expectedPreviousBlock) {
			t.Errorf("expected previous block to be %x, got %x", expectedPreviousBlock, block.PreviousBlock)
		}

		expectedMerkleRoot := []byte{0xe5, 0x68, 0x58, 0x3d, 0x0e, 0xfc, 0x40, 0x04, 0x10, 0x40, 0x2a, 0x9f, 0x74, 0x42, 0xe4, 0x5a, 0x80, 0x41, 0xb0, 0x54, 0x12, 0x46, 0xfd, 0xe8, 0xaa, 0xd9, 0x80, 0xd0, 0x29, 0xde, 0xca, 0xbf}
		if !bytes.Equal(block.MerkleRoot[:], expectedMerkleRoot) {
			t.Errorf("expected merkle root to be %x, got %x", expectedMerkleRoot, block.MerkleRoot)
		}
